...
const Submission = ({ route }) => {
  ...
  // Check if the user has already submitted for this mission
  useEffect(() => {
    if (userName && missionId) {
      const db = getDatabase();
      const submissionRef = ref(db, `submissions/${userName}/${missionId}`); // Use missionId as part of the key

      get(submissionRef)
        .then((snapshot) => {
          if (snapshot.exists()) {
            setIsSubmitted(true); // Mark as submitted if the user has already submitted for this mission
          }
        })
        .catch((error) => {
