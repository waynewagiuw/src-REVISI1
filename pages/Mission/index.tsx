  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(missionsRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        // Urutkan berdasarkan `postedAt` secara descending (terbaru muncul di atas)
        missionList.sort((a, b) => (b.postedAt || 0) - (a.postedAt || 0));
          ...
          {missions.map((mission) => {
            ...
            return (
              <SubMission
                ...
                status={status}
                quota={quota}
                expire={mission.availableUntil} // Pastikan properti ini ada
                supervisor={mission.supervisor} // Pastikan properti ini ada
                onPress={() => handleImagePress(mission)}
              />
