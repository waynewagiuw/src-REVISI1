...
const TerimaMisi = ({ navigation, route }) => {
  ...
  const handleYa = () => {
    ...
    if (currentAccepted < quota) {
      ...
      update(missionRef, {
        status: newStatus,
        isAccepted: true, // Tandai misi sebagai diterima
      })
        .then(() => {
          alert('Misi diterima!');
          setShowPopup(false);
          navigation.navigate('Submission', { missionId }); // Navigasi ke halaman Submission
        })
        .catch((error) => {
  ...
  const handleTidak = () => {
    setShowPopup(false);
    navigation.navigate('MissionDetail', { missionId }); // Navigasi kembali ke MissionDetail
  };
